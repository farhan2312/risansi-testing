...
export async function POST(req: Request) {
  ...
  if (!user || !(await bcrypt.compare(password, user.passwordHash))) {
    ...
    await logAudit({ eventType: "login_failed", userEmail: email, details: "Invalid email or password" });
  ...
  await logAudit({ userId: user.id, userName: user.name, userEmail: user.email, eventType: "login" });
